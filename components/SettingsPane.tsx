...
import type { ToolConfiguration, Project, AlertConfiguration } from '../types';
import { ConnectionStatus, Tool, DefectSeverity, TaskPriority } from '../types';
...
export const SettingsPane: React.FC<SettingsPaneProps> = ({ configurations, setConfigurations, projects, setProjects, alertConfig, setAlertConfig }) => {
    ...
    const handleSeverityToggle = (severity: DefectSeverity) => {
        setAlertConfig(prev => {
            const newSeverities = prev.defectSeverities.includes(severity)
                ? prev.defectSeverities.filter(s => s !== severity)
                : [...prev.defectSeverities, severity];
            return { ...prev, defectSeverities: newSeverities };
        });
    };
    
    const handlePriorityToggle = (priority: TaskPriority) => {
        setAlertConfig(prev => {
            const newPriorities = prev.taskPriorities.includes(priority)
                ? prev.taskPriorities.filter(p => p !== priority)
                : [...prev.taskPriorities, priority];
            return { ...prev, taskPriorities: newPriorities };
        });
    };
    ...
    return (
        <>
            ...
            <div className="p-4">
                <h3 className="font-semibold text-slate-800 dark:text-slate-100 mb-2">Real-time Alert Configuration</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">Select which items should trigger an immediate notification.</p>
                
                <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-md space-y-4">
                    <div>
                        <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Notify for Defect Severities:</h4>
                        <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                            {Object.values(DefectSeverity).map(severity => (
                                <label key={severity} className="flex items-center space-x-2 text-sm text-slate-600 dark:text-slate-300 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={alertConfig.defectSeverities.includes(severity)}
                    ...
                    <div>
                        <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Notify for Task Priorities:</h4>
                         <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                            {Object.values(TaskPriority).map(priority => (
                                <label key={priority} className="flex items-center space-x-2 text-sm text-slate-600 dark:text-slate-300 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={alertConfig.taskPriorities.includes(priority)}
